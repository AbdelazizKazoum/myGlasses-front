...
const PaymentPage = ({ updateDisplayPayment, updatePaymentSuccess }) => {
  ...
  // Define validation schema dynamically based on the selected payment method
  ...
  useEffect(() => {
    ...
    reset(); // Reset form fields when payment method changes
  }, [paymentMethod]);
              ...
              <button
                ...
                className={`flex items-center justify-center space-x-2 p-4 rounded-lg border transition ${
                  paymentMethod === method.id
                    ? "border border-primary-500 bg-green-500 text-white"
